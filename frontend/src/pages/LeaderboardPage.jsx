        ...
        <div className="bg-white rounded-2xl overflow-hidden shadow-card">
          ...
          {leaders.map(l => { const isMe = authUser?.id === l.id; return (
            <Link key={l.id} to={`/profile/${l.id}`}
              ...
              <div className="col-span-6 flex items-center gap-2 min-w-0"><span className="text-app-text font-medium truncate">{getDisplayName(l)}</span>{isMe && <span className="px-1.5 py-0.5 rounded text-[10px] bg-app-blue/10 text-app-blue font-medium">你</span>}</div>
